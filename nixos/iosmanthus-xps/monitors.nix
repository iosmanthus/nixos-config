{ config
, ...
}:
let
  name = "eDP-1-1";
  fingerprint = "";
in
{
  monitors = {
    builtin = {
      inherit name fingerprint;
      ports = builtins.map (p: "DP-1-" + (builtins.toString p)) [ 1 2 3 4 ];
    };
  };
  services.autorandr = {
    enable = true;
    profiles = {
      default = {
        fingerprint = {
          "${name}" = fingerprint;
        };
        config = {
          "${name}" = {
            enable = true;
            mode = "3840x2400";
          };
        };
      };

      home-dual-1-2 = {
        fingerprint = {
          "${name}" = fingerprint;
          "DP-1-1" = config.monitors.external.home-lg.fingerprint;
          "DP-1-2" = config.monitors.external.home-aoc.fingerprint;
        };
        config = {
          "${name}".enable = false;
          "DP-1-2" = {
            enable = true;
            primary = true;
            position = "3840x0";
            mode = "3840x2160";
            rate = "60.00";
          };
          "DP-1-1" = {
            enable = true;
            primary = false;
            position = "0x0";
            mode = "2560x1440";
            rate = "60.00";
            rotate = "left";
          };
        };
      };
    };
  };
}
