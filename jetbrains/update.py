...
versions_file_path = pathlib.Path(__file__).parent.joinpath(
    "versions.json").resolve()
...
def download_channels():
    ...
    return {
        ...
        for product in products for channel in one_or_more(product["channel"])
    }


def build_version(build):
    build_number = build["@fullNumber"] if "@fullNumber" in build else build[
        "@number"]
...
def update_product(name, product):
    ...
    if channel is None:
        ...
        logging.error(
            ...
            versions_file_path, updates_url)
    else:
        try:
            ...
            download_url = product["url-template"].format(
                version=version_or_build_number,
                versionMajorMinor=version_number)
            product["url"] = download_url
            if "sha256" not in product or product.get(
                    "build_number") != new_build_number:
                logging.info("Found a newer version %s with build number %s.",
