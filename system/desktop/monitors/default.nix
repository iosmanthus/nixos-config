{ config
, pkgs
, ...
}:
let
  builtinDisplayPort = config.machine.displayPorts.builtin.name;
  builtinDisplayPortFingerprint = config.machine.displayPorts.builtin.fingerprint;
  inherit (config.machine.displayPorts) extra;
  restartPolybar = ''
    ${pkgs.systemd}/bin/systemctl restart --user polybar.service
  '';
  hooks = {
    postswitch = {
      inherit restartPolybar;
    };
  };
  mk4k = p: {
    "${builtinDisplayPort}".enable = false;
    "${p}" = {
      enable = true;
      primary = true;
      mode = "3840x2160";
      rate = "60.00";
    };
  };
  mkProfile =
    { name
    , fingerprint
    , mkConfig
    , extra
    }: builtins.foldl'
      (acc: p: acc // {
        "${name}-${p}" = {
          fingerprint = {
            "${builtinDisplayPort}" = builtinDisplayPortFingerprint;
            "${p}" = fingerprint;
          };
          config = mkConfig p;
          inherit hooks;
        };
      })
      { }
      extra;
in
{
  imports = [
    ./devices.nix
  ];

  monitors = {
    home-aoc.fingerprint = "";
    office-lg.fingerprint = "";
    home-lg.fingerprint = "";
  };

  services.autorandr = {
    enable = true;
    inherit hooks;
    profiles = mkProfile
      {
        name = "aoc-only";
        inherit (config.monitors.home-aoc) fingerprint;
        inherit extra;
        mkConfig = mk4k;
      }
    // mkProfile
      {
        name = "lg-only";
        inherit (config.monitors.home-lg) fingerprint;
        inherit extra;
        mkConfig = mk4k;
      }
    // mkProfile
      {
        name = "office";
        inherit (config.monitors.office-lg) fingerprint;
        inherit extra;
        mkConfig = mk4k;
      };
  };
}
